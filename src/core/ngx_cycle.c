...
/*
lgf:
old_cycle : 临时ngx_cycle_t 指针
function: 负责初始化ngx_cycle_t 中数据结构体、解析配置文件、打开监听端口、初始化进程通信等
*/
...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
	...
	//lgf:调用所有核心模块的create_conf方法 所有核心模块开始构造用于存储配置项的结构体
   ...
   //----------开始解析配置---cycle->conf_file文件名-----
	...
	/*lgf 核心模块在解析完配置后 做综合性处理(init_conf) */
    for (i = 0; ngx_modules[i]; i++) {
        if (ngx_modules[i]->type != NGX_CORE_MODULE) {
            continue;
        }
